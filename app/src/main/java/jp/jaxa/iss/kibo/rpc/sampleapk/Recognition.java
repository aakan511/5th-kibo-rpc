...
//            "kapton_tape", "screwdriver", "thermometer", "top", "watch", "wrench"};
    ...
    public Recognition(Context context, int id, String[] names, KiboRpcApi api) {
    ...
    public List<Detection> filterDetections(float[] results, float confidenceThreshold, int originalWidth, int originalHeight) {
        ...
        for (int i = 0; i < numDetections; i++) {
            ...
            if (confidence >= confidenceThreshold) {
                ...
                detections.add(new Detection(confidence, new Rect(x, y, width, height), classId, classNames[classId]));
            }
        }
    ...
    public RecognitionResult findTarget(Mat image, int id) {
        int origHeight = image.rows();
        int origWidth = image.cols();
        ...
        try {
            ...
            List<Detection> detections = filterDetections(results, confidenceThreshold, origWidth, origHeight);
            drawBoundingBoxes(image, detections);
            api.saveMatImage(image, "recognitionTesting" + id + ".jpg");

            String className = "beaker";
            int count = detections.size();
            float maxConfidence = 0.0f;

            for (int i = 0; i < count; i++) {
                if (detections.get(i).confidence >= maxConfidence) {
                    maxConfidence = detections.get(i).confidence;
                    className = detections.get(i).className;
                }
            }

            return new RecognitionResult(image, count, className);
        } catch (Exception e) {
            Log.e("Recognition", "Identification Error", e);
        }
        return new RecognitionResult(image, 0, "beaker");
    }
